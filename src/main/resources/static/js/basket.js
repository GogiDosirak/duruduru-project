let BasketObject = {
		...
		$(document).on("click", ".btn-updateBasket", function() {
			...
			$("#btn-insertBasket").on("click", () => {
				_this.insertBasket();
					...
					$("#btn-insertOneBasket").on("click", () => {
				_this.insertOneBasket();
	...
	insertOneBasket: function() {


		let insertOneBasketData = {
			productAmount: $("#productAmount").val(),
			productSeq: $("#productSeq").val(),
			userSeq: $("#userSeq").val()
		}
		// 객체를 폼 데이터 형태로 직렬화
		let formData = $.param(insertOneBasketData);
		...
		$.ajax({
			...
			alert(message);
			if (response.status === 200) {
				location = "/order/" + insertOneBasketData.userSeq;
			}
			else {
				location = "/mall";
			}
